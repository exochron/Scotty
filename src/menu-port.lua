...
menuActionButton:SetAttributeNoHandler("pressAndHoldAction", 1)
...
local function buildEntry(menuRoot, dbType, typeId, icon, location, tooltipSetter, hasCooldown, dbRow)
    ...
    element:HookOnEnter(function(frame)
        ...
        menuActionButton:SetAttributeNoHandler("type", dbType)
        menuActionButton:SetAttributeNoHandler("typerelease", dbType)
        menuActionButton:SetAttributeNoHandler(dbType, typeId)
    ...
    if portalId and C_SpellBook.IsSpellInSpellBook(portalId) then
        element:AddInitializer(function(button, elementDescription, menu)
            ...
            portalButton:SetScript("OnEnter", function()
                ...
                menuActionButton:SetAttributeNoHandler("spell", portalId)
            ...
            portalButton:SetScript("OnLeave", function()
                ...
                menuActionButton:SetAttributeNoHandler("spell", spellId)
                button.highlight:ClearAllPoints()
...
local function buildRow(row, menuRoot)
    ...
    elseif row.neighborhoodGUID and row.houseGUID and row.plotID then
        ...
        button:HookOnEnter(function()
            menuActionButton:SetAttributeNoHandler("house-neighborhood-guid", row.neighborhoodGUID)
            menuActionButton:SetAttributeNoHandler("house-guid", row.houseGUID)
            menuActionButton:SetAttributeNoHandler("house-plot-id", row.plotID)
        end)
    end
end
...
local function generateTeleportMenu(_, root)
    ...
    do
        ...
        for _, playerHouse in ipairs(playerHouseInfos) do
            if C_HousingNeighborhood.CanReturnAfterVisitingHouse() and C_Housing.GetCurrentNeighborhoodGUID() == playerHouse.neighborhoodGUID then
                ...
                button:HookOnEnter(function()
                    menuActionButton:SetAttributeNoHandler("house-neighborhood-guid", playerHouse.neighborhoodGUID)
                    menuActionButton:SetAttributeNoHandler("house-guid", playerHouse.houseGUID)
                    menuActionButton:SetAttributeNoHandler("house-plot-id", playerHouse.plotID)
                end)
            end
    ...
    if Settings.GetValue(ADDON_NAME.."_SHOW_FRIENDS_HOUSES") and TableHasAnyEntries(friendsHouseInfos) then
        ...
        for _, houseInfo in ipairs(friendsHouses) do
            ...
            button:HookOnEnter(function()
                menuActionButton:SetAttributeNoHandler("house-neighborhood-guid", houseInfo.neighborhoodGUID)
                menuActionButton:SetAttributeNoHandler("house-guid", houseInfo.houseGUID)
                menuActionButton:SetAttributeNoHandler("house-plot-id", houseInfo.plotID)
            end)
        end
